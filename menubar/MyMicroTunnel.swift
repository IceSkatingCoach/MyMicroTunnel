...
struct Tunnel: Decodable {
    ...
    /// False for a profile that has been saved but never deployed. It has no
    /// stack, no gateway and nothing to connect to, and saying so is better
    /// than leaving it out of the menu — which read as the save having been
    /// lost.
    var deployed = false
    ...
    static func installed() -> [Tunnel] {
        ...
        for directory in contents.sorted(by: { $0.lastPathComponent < $1.lastPathComponent }) {
            ...
            // config.json is written when a profile is deployed; settings.json
            // as soon as it is saved. A draft decodes from the second because
            // the field names it shares are the ones this needs.
            let deployed = FileManager.default.fileExists(atPath: config.path)
            let source = deployed ? config : settings
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    /// One line per VPN profile, carrying its state, with everything that
    /// acts on that profile in a submenu beneath it.
    ///
    /// Always a submenu, even for one profile: a flat list of identical verbs
    /// is one where the wrong Connect moves somebody else's tunnel, and a
    /// layout that rearranges itself when a second profile appears is one
    /// nobody can learn.
    private func addProfiles(to menu: NSMenu) {
        for profile in profiles {
            ...
            // Ticked when up, so the whole machine reads at a glance without
            // opening anything.
            header.state = isConnected(profile) ? .on : .off
            header.submenu = submenu(for: profile)
            menu.addItem(header)
        }
    }
    ...
    private func submenu(for profile: Tunnel) -> NSMenu {
        let menu = NSMenu()

        // A saved profile with no stack has nothing to connect to. Saying so
        // here is the whole reason it is listed at all.
        guard profile.deployed else {
            menu.addItem(disabledItem("Saved, but never deployed"))
            menu.addItem(disabledItem("Open Setup… and press Deploy CloudFormation"))
            return menu
        }
        ...
        for item in items {
            // Every item carries the profile's name, so an action can never
            // reach the wrong tunnel.
            item.target = self
